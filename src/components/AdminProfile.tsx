...
const AdminProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const normalizeProfileResponse = (
      ...
      const organization =
        raw?.organization ?? raw?.org ?? raw?.organization_info;
      ...
      const normalizedEvents: AdminEvent[] = upcoming.map(
        (event: any, idx: number) => ({
          ...
          start_time:
            event?.start_time ?? event?.start ?? event?.startTime ?? "00:00",
      ...
      return {
        admin: {
          ...
          organization_id:
            admin?.org_id ?? admin?.organization_id ?? organization?.id,
          organization_name:
            organization?.name ??
  ...
  const memberSince = parseLocalDate(admin.created_at) ?? new Date();
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <p className="text-gray-600">
          <strong>Member Since:</strong> {formatLocalDate(memberSince)}
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-blue-50 p-4 rounded-lg">
            <h3 className="text-2xl font-bold text-blue-700">
              {organization_stats.total_volunteers}
            </h3>
            <p className="text-gray-600">Total Volunteers</p>
          </div>
          <div className="bg-green-50 p-4 rounded-lg">
            <h3 className="text-2xl font-bold text-green-700">
              {organization_stats.total_events}
            </h3>
            <p className="text-gray-600">Total Events</p>
          </div>
          <div className="bg-yellow-50 p-4 rounded-lg">
            <h3 className="text-2xl font-bold text-yellow-700">
              {organization_stats.upcoming_events}
            </h3>
            <p className="text-gray-600">Upcoming Events</p>
          </div>
          <div className="bg-purple-50 p-4 rounded-lg">
            <h3 className="text-2xl font-bold text-purple-700">
              {organization_stats.volunteer_hours}
            </h3>
              ...
              >
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-gray-600">
                      {formatLocalDate(event.day)} • {event.start_time} -{" "}
                      {event.end_time}
                    </p>
